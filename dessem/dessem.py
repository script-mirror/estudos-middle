...
from PMO.scripts_unificados.bibliotecas import opweek,wx_ssh,wx_dbUpdater
import operuh, entdados, deflant, dadvaz, operut, respot, dessemBase, renovaveis, cotasr11, configs, relatorioIntercambio, ferramentaAuxiliar, dessem.converte_ons_to_ccee as converte_ons_to_ccee
from PMO.scripts_unificados.apps.dbUpdater.libs import deck_ds


configs = configs.getConfiguration()
...
def converter_deck_ons_para_ccee(data):
    ...
    path_deck_convertido = converte_ons_to_ccee.converter_entdados(data)
...
def prepararArquivosAuxiliares(data, preliminar=True):
    ...
    ultimoSabado = opweek.getLastSaturday(data)
    dataEletrica =  opweek.ElecData(ultimoSabado)
...
def gerarArquivosEntrada(data):
    ...
    entdados.gerarEntdados(data, pathArquivos)
    operuh.gerarOperuh(data, pathArquivos)
    operut.gerarOperut(data, pathArquivos)
    deflant.gerarDeflant(data, pathArquivos)
    renovaveis.gerarRenovaveis(data, pathArquivos)
    dadvaz.gerarDadvaz(data, pathArquivos)
    respot.gerarRespot(data, pathArquivos)
    cotasr11.gerarCotasr11(data, pathArquivos)

def gerarArquivosDefinitivos(data):
    global pathArquivos

    prepararArquivosAuxiliares(data, preliminar=False)
    entdados.gerarEntdadosDef(data, pathArquivos)
    operuh.gerarOperuhDef(data, pathArquivos)
    operut.gerarOperutDef(data, pathArquivos)
    # deflant.gerarDeflantDef(data, pathArquivos)
    renovaveis.gerarRenovaveisDef(data, pathArquivos)
    # dadvaz.gerarDadvazDef(data, pathArquivos)
    # respot.gerarRespotDef(data, pathArquivos)
    # cotasr11.gerarCotasr11Def(data, pathArquivos)
...
def importarResultados(pdocmoPath, dtRodada, dtReferente, fonte='wx', comentario='', enviarEmail=False):
    ...
    dbUpdater.insertRodadaDessem(pdocmoPath, dtRodada, dtReferente, fonte, comentatio='')
    ...
    dbUpdater.importar_renovaveis_dessem(data_deck=data_referente, path_arquivo=path_renovaveis, fonte=fonte)
...
def baixarArquivosCcee(data, pathArquivos, importarDb=False, enviarEmail=False):
    for tentativa in range(40):
        ...
        pathArquivoDessem = dessemBase.downloadArquivoBase(pathArquivos, dtAtual=data)
        if dessemBase.organizarArquivoBase(data, pathArquivoDessem):
            break
...
def atualizarHistoricoFaltanteCcee(pathZipFile):
    ...
    dtInicial = opweek.getLastSaturday(diaPrimeiro)

    primeiroDiaProxRv = opweek.getLastSaturday(diaPrimeiro + datetime.timedelta(days=31))
    ...
    for dt in dtasAtualizar:
        if dt in [datetime.date(2022,1,2),datetime.date(2022,1,8)]:
            continue
        dtElec = opweek.ElecData(wx_opweek.getLastSaturday(dt))
...
def printHelper():
    ...
    ultimoSabado = opweek.ElecData(ultimoSabado.date())
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if 'gerarRelatorioIntercambio' in sys.argv:
            ...
            relatorioIntercambio.analisarIntercambios(pathArquivosEntradaDessem, pathArquivosSaidaDessem)
