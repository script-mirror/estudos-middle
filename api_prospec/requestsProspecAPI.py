...
def getInfoFromAPI(*args):
    ...
    if int(response.status_code) != 200 and int(response.status_code) != 201 and int(response.status_code) != 401: 
        print('Erro na execução, codigo: ' + str(response.status_code))
        print(response.text)
        sys.exit()
...
def sendFileToAPI(*args):
    ...
    if len(args) < 4:
        print('Sao necessarios quatro argumentos: token, apiFunction pathToFile e nameToFile')
        return ''
    elif len(args) == 4:
        token = args[0]
        apiFunction = args[1]
